...
void AnycubicTFTClass::StartPrint() {
	if (TFTstate == ANYCUBIC_TFT_STATE_SDPAUSE || TFTstate == ANYCUBIC_TFT_STATE_SDPAUSE_M600) {
...
void AnycubicTFTClass::PausePrint() {
	bool m108Stop = strcmp(command, "M108") == 0;
	if (m108Stop)
	{
		enqueue_and_echo_commands_P(PSTR("M600"));
#ifdef ANYCUBIC_TFT_DEBUG
		SERIAL_ECHOLNPGM("TFT Serial Debug: M600 initiated");
#endif
		TFTstate = ANYCUBIC_TFT_STATE_SDPAUSE_M108;
	}
	else
	{
#ifdef SDSUPPORT
...
void AnycubicTFTClass::StateHandler()
{
	...
	case ANYCUBIC_TFT_STATE_SDPAUSE:
		break;
	case ANYCUBIC_TFT_STATE_SDPAUSE_M600:
		if (card.sdprinting) {
			TFTstate = ANYCUBIC_TFT_STATE_SDPRINT;
#ifdef ANYCUBIC_TFT_DEBUG
			...
			serial3_count >= (TFT_MAX_CMD_SIZE - 1))
		{
			...
			if ((strchr(TFTcmdbuffer[TFTbufindw], 'A') != NULL)) {
				...
				case 10: // A10 resume sd print
					...
					else if (ANYCUBIC_TFT_STATE_SDPAUSE_M600)
					{
						//wait_for_user = false;
						ResumePrint();
#ifdef ANYCUBIC_TFT_DEBUG
...
#ifdef SDSUPPORT		  
					if ((!planner.movesplanned()) && (TFTstate != ANYCUBIC_TFT_STATE_SDPAUSE) && (TFTstate1 = ANYCUBIC_TFT_STATE_SDPAUSE_M600) && (TFTstate != ANYCUBIC_TFT_STATE_SDOUTAGE) && (card.isFileOpen()))
					{
						StartPrint();
